...
拆解需求点：
1. 顺序执行 migration 文件：对文件名称格式化后按顺序进行执行
2. 文件内容：根据文件名称执行对应的操作（规范文件名）此时一个文件中内容是规范好的，或在一个文件中包含多个操作；最终选择一个文件执行一个操作更易理解

大盘的增删改和规则的增删改不一样，但又都有类似的结构，所以我们可以创建一个抽象基类 BaseMigration，抽象方法主要是 upsert 和 delete 普通方法是：执行动作
执行 migrate 动作：将每个动作抽象成 action，收集该类 migrate 下的文件涉及到的所有 action 顺序执行

后续可能增加更多类的 migrate，为了便于扩展再构造一个类来作为迁移框架的主入口即 Migrate 类
"""
import re
